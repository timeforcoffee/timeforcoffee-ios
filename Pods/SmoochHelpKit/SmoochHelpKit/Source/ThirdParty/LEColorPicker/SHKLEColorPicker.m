...
@implementation SHKLEColorScheme
@end

@implementation SHKLEColorPicker

#pragma mark - Preprocessor definitions
#define SHKLECOLORPICKER_GPU_DEFAULT_SCALED_SIZE                           32
#define SHKLECOLORPICKER_BACKGROUND_FILTER_TOLERANCE                       0.5
#define SHKLECOLORPICKER_PRIMARY_TEXT_FILTER_TOLERANCE                     0.3
#define SHKLECOLORPICKER_DEFAULT_COLOR_DIFFERENCE                          500
#define SHKLECOLORPICKER_DEFAULT_BRIGHTNESS_DIFFERENCE                     125

#define SHKSTRINGIZE(x) #x
#define SHKSTRINGIZE2(x) SHKSTRINGIZE(x)
#define SHKSHADER_STRING(text) @ SHKSTRINGIZE2(text)
...
static NSString *const kDominantVertexShaderString = SHKSHADER_STRING
(
...
static NSString *const kDominantFragmentShaderString = SHKSHADER_STRING
(
 ...
 */
static unsigned int squareDistanceInRGBSpaceBetweenColor(LEColor colorA, LEColor colorB);
...
static unsigned int squareDistanceInRGBSpaceBetweenColor(LEColor colorA, LEColor colorB)
{
    ...
    return (unsigned int)squareDistance;
}
...
- (void)pickColorsFromImage:(UIImage *)image
                 onComplete:(void (^)(SHKLEColorScheme *colorsPickedDictionary))completeBlock
{
    if ([self isAppActive]) {
        dispatch_async(taskQueue, ^{
            // Color calculation process
            SHKLEColorScheme *colorScheme = [self colorSchemeFromImage:image];
...
- (SHKLEColorScheme*)colorSchemeFromImage:(UIImage*)inputImage
{
    if ([self isAppActive]) {
        ...
        UIImage *scaledImage = [self scaleImage:inputImage
                                          width:SHKLECOLORPICKER_GPU_DEFAULT_SCALED_SIZE
                                         height:SHKLECOLORPICKER_GPU_DEFAULT_SCALED_SIZE];
        ...
        SHKLEColorScheme *colorScheme = [[SHKLEColorScheme alloc] init];
        colorScheme.backgroundColor = [self colorWithBiggerCountFromImageWidth:SHKLECOLORPICKER_GPU_DEFAULT_SCALED_SIZE height:SHKLECOLORPICKER_GPU_DEFAULT_SCALED_SIZE];
...
{
    static SHKLEColorPicker *colorPicker;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        colorPicker = [[SHKLEColorPicker alloc] init];
    });
    
    [colorPicker pickColorsFromImage:image onComplete:^(SHKLEColorScheme *colorScheme) {
...
- (void)renderDominant
{
    ...
    //Setup inputs
    glViewport(0, 0, SHKLECOLORPICKER_GPU_DEFAULT_SCALED_SIZE, SHKLECOLORPICKER_GPU_DEFAULT_SCALED_SIZE);
    ...
    glUniform1i(_proccesedWidthSlot, SHKLECOLORPICKER_GPU_DEFAULT_SCALED_SIZE/2);
    glUniform1i(_totalWidthSlot, SHKLECOLORPICKER_GPU_DEFAULT_SCALED_SIZE);
...
- (void)setupRenderBuffer {
    ...
    glRenderbufferStorage(GL_RENDERBUFFER, GL_RGBA8_OES, SHKLECOLORPICKER_GPU_DEFAULT_SCALED_SIZE, SHKLECOLORPICKER_GPU_DEFAULT_SCALED_SIZE);
...
- (void)setupDepthBuffer {
    ...
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT16, SHKLECOLORPICKER_GPU_DEFAULT_SCALED_SIZE , SHKLECOLORPICKER_GPU_DEFAULT_SCALED_SIZE);
...
-(void)findTextColorsTaskForColorScheme:(SHKLEColorScheme*)colorScheme
{
    //Set sizes for buffer index calculations
    NSUInteger width = SHKLECOLORPICKER_GPU_DEFAULT_SCALED_SIZE;
    NSUInteger height = SHKLECOLORPICKER_GPU_DEFAULT_SCALED_SIZE;
    ...
    for (NSUInteger y=0; y<(height/2); y++) {
        for (NSUInteger x=0; x<(width/2)*4; x++) {
            if ((!((x+1)%4)) && (x>0)) {
                ...
                LEColor currentColor = {(int)currentRed, (int)currentGreen, (int)currentBlue};
                NSUInteger squareDistance = squareDistanceInRGBSpaceBetweenColor(currentColor, backgroundColor);
                NSUInteger thresholdSquareDistance = (255*SHKLECOLORPICKER_BACKGROUND_FILTER_TOLERANCE)*(255*SHKLECOLORPICKER_BACKGROUND_FILTER_TOLERANCE);
    ...
    LEColor primaryTextColor = {(int)primaryColorR, (int)primaryColorG, (int)primaryColorB};
    for (NSUInteger y=0; y<(height/2); y++) {
        for (NSUInteger x=0; x<(width/2)*4; x++) {
            if ((!((x+1)%4)) && (x>0)) {
                NSUInteger currentRed = buffer[y * 4 * width + (x-3)];
                ...
                NSUInteger thresholdSquareDistanceToBackground = (255*SHKLECOLORPICKER_BACKGROUND_FILTER_TOLERANCE)*(255*SHKLECOLORPICKER_BACKGROUND_FILTER_TOLERANCE);
                NSUInteger thresholdSquareDistanceToPrimary = (255*SHKLECOLORPICKER_PRIMARY_TEXT_FILTER_TOLERANCE)*(255*SHKLECOLORPICKER_PRIMARY_TEXT_FILTER_TOLERANCE);
    ...
    if ([self isSufficienteContrastBetweenBackground:colorScheme.backgroundColor
        ...
        if ([SHKUIColorExtensions yComponentFromColor:colorScheme.backgroundColor] < 0.5) {
...
{
    float backgroundColorBrightness = [SHKUIColorExtensions yComponentFromColor:backgroundColor];
    float foregroundColorBrightness = [SHKUIColorExtensions yComponentFromColor:foregroundColor];
    ...
    if (brightnessDifference>=SHKLECOLORPICKER_DEFAULT_BRIGHTNESS_DIFFERENCE) {
        ...
        numComponents = (int)CGColorGetNumberOfComponents(foregroundColor.CGColor);
        ...
        if ((colorDifference*255)>SHKLECOLORPICKER_DEFAULT_COLOR_DIFFERENCE) {
            return YES;
        }
    }
...
// ------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------
// Smooch is doing it's own handling of this
// ------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------
