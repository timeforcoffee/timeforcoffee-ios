...
@implementation SHKHTMLNode

-(SHKHTMLNode*)parent
{
	return [[SHKHTMLNode alloc] initWithXMLNode:_node->parent];	
}

-(SHKHTMLNode*)nextSibling {
	return [[SHKHTMLNode alloc] initWithXMLNode:_node->next];
}

-(SHKHTMLNode*)previousSibling {
	return [[SHKHTMLNode alloc] initWithXMLNode:_node->prev];
}
...
{	
	...
	return skGetAttributeNamed(_node, nameStr);
}
...
{
	return [[SHKHTMLNode alloc] initWithXMLNode:_node->children];
}
		...
		for(xmlAttrPtr attr = cur_node->properties; NULL != attr; attr = attr->next)
		{
			...
			{				
				for(xmlNode * child = attr->children; NULL != child; child = child->next)
				{
					...
					{
						//Found node
						SHKHTMLNode * nNode = [[SHKHTMLNode alloc] initWithXMLNode:cur_node];
...
-(void)findChildTags:(NSString*)tagName inXMLNode:(xmlNode *)node inArray:(NSMutableArray*)array
{
	...
	{				
		...
		{
			SHKHTMLNode * node = [[SHKHTMLNode alloc] initWithXMLNode:cur_node];
...
-(SHKHTMLNode*)findChildTag:(NSString*)tagName inXMLNode:(xmlNode *)node
{
	xmlNode *cur_node = NULL;
	const char * tagNameStr =  [tagName UTF8String];
	...
	{				
		...
		{
			return [[SHKHTMLNode alloc] initWithXMLNode:cur_node];
		}
		
		SHKHTMLNode * cNode = [self findChildTag:tagName inXMLNode:cur_node->children];
		if (cNode != NULL)
		{
			return cNode;
		}
...
-(SHKHTMLNode*)findChildWithAttribute:(const char*)attribute matchingName:(const char*)name inXMLNode:(xmlNode *)node allowPartial:(BOOL)partial
{
	xmlNode *cur_node = NULL;
	...
	{		
		...
		SHKHTMLNode * cNode = [self findChildWithAttribute:attribute matchingName:name inXMLNode:cur_node->children allowPartial:partial];
		if (cNode != NULL)
		{
			return cNode;
		}
...
{	
	SHKHTMLNode * node = [self findChildWithAttribute:"class" matchingName:[className UTF8String]  inXMLNode:_node->children allowPartial:NO];
	return node;
}
...
SHKHTMLNodeType shkNodeType(xmlNode * _node)
{
...
{
	return skAllNodeContents(_node);
}
...
-(NSString*)rawContents {
	return skRawContentsOfNode(_node);
}
